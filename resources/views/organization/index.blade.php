                                ...
                                <div class="mt-8 p-6 bg-slate-700/30 rounded-lg" x-init="loadDriveSubfolders(org)">
                                    ...
                                    <div x-show="getDriveState(org.id).connected" class="space-y-4">
                                        ...
                                        <div class="mt-4 space-y-3">
                                            <h4 class="font-semibold mb-1">Organización automática</h4>
                                            <p class="text-slate-400">Las carpetas internas para audios y transcripciones se crean y sincronizan automáticamente.</p>
                                            <div x-show="!getDriveState(org.id).isLoading && getDriveState(org.id).standardFolders.length"
                                                 class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                                                <template x-for="sf in getDriveState(org.id).standardFolders" :key="sf.google_id">
                                                    <div class="flex items-center justify-between p-3 rounded-lg bg-slate-800/60 border border-slate-700/50">
                                                        <span class="font-medium text-slate-200" x-text="sf.name"></span>
                                                        <span class="text-xs text-slate-400 font-mono" x-text="sf.google_id"></span>
                                                    </div>
                                                </template>
                                            </div>
                                            <p x-show="!getDriveState(org.id).isLoading && !getDriveState(org.id).standardFolders.length"
                                               class="text-slate-500">Se crearán automáticamente cuando se sincronicen tus reuniones.</p>
                                        </div>
                                    </div>
                                </div>
                            </div>
