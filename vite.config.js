import { defineConfig } from 'vite';
import laravel from 'laravel-vite-plugin';

export default defineConfig({
    server: {
        host: '127.0.0.1',
        port: 5175,
        cors: true
    },
    optimizeDeps: {
        exclude: ['@ffmpeg/ffmpeg', '@ffmpeg/util']
    },
    plugins: [
        laravel({
            input: [
                'resources/css/app.css',
                'resources/js/app.js',
                'resources/css/index.css',
                'resources/js/index.js',
                'resources/css/profile.css',
                'resources/js/profile.js',
                'resources/css/auth/login.css',
                'resources/js/auth/login.js',
                'resources/css/auth/register.css',
                'resources/js/auth/register.js',
                'resources/css/reuniones_v2.css',
                'resources/js/reuniones_v2.js',
                'resources/js/reuniones/globals.js',
                'resources/js/reuniones/init.js',
                'resources/css/new-meeting.css',
                'resources/js/meetings/new-meeting-globals.js',
                'resources/js/new-meeting.js',
                'resources/css/meetings/new-meeting-nocors.css',
                'resources/js/meetings/new-meeting-nocors.js',
                'resources/css/audio-processing.css',
                'resources/js/audio-processing.js',
                'resources/js/notifications.js',
                'resources/css/tasks.css',
                'resources/js/tasks/tabs-tareas.js',
                'resources/css/organization.css',
                'resources/js/organization.js',
                'resources/js/tests/ffmpeg-test.js',
                'resources/js/tests/ffmpeg-test-simple.js',
                'resources/js/tests/test-notifications.js',
                'resources/js/tests/test-upload.js',
                'resources/js/tests/test-pending.js',
                'resources/js/tests/test-memory-cleanup.js',
                'resources/js/tests/test-mp3-nocors.js',
                'resources/css/tests/test-memory-cleanup.css',
                'resources/css/tests/test-mp3-nocors.css',
                'resources/js/admin/dashboard.js',
                'resources/js/ffmpeg-test.js',
                'resources/js/ffmpeg-test-simple.js',
                'resources/js/tasks/tabs-reuniones.js',

            ],
            refresh: true,
        }),
    ],
});
